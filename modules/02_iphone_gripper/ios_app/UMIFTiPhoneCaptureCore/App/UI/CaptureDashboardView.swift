...
struct CaptureDashboardView: View {
    ...
    @State private var didRunAutoRecording = false
    ...
    var body: some View {
        GeometryReader { geometry in
            HStack(spacing: 0) {
                ...
                ParameterPanel(onPrimaryAction: handlePrimaryAction)
                    .frame(width: geometry.size.width * 0.5, height: geometry.size.height)
        ...
        .onAppear {
            worldARActive = true
            arCapture.setRecordingActive(appState.phase == .recording)
            arCapture.setRecordingBufferEnabled(shouldEnableRecordingBuffer(for: appState.phase))
        ...
        .onChange(of: appState.phase) { oldPhase, newPhase in
            ...
            arCapture.setRecordingActive(newPhase == .recording)
            arCapture.setRecordingBufferEnabled(shouldEnableRecordingBuffer(for: newPhase))
            runAutoRecordingIfReady(phase: newPhase)
    ...
    private func runAutoRecordingIfReady(phase: CapturePhase) {
        guard phase == .previewCalibrated,
              !didRunAutoRecording,
              let durationS = DebugAutomation.autoRecordDurationS
        else { return }
        didRunAutoRecording = true
        print("[UMIFT][automation] auto recording scheduled duration=\(durationS)s")
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.75) {
            guard appState.phase == .previewCalibrated else { return }
            appState.beginRecordCountdown()
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.5 + durationS) {
                guard appState.phase == .recording else { return }
                appState.terminateRecording()
                print("[UMIFT][automation] auto recording stopped")
            }
        }
    }
    ...
    private func runAutoWorldOriginMarkIfReady(now: Date = Date()) {
        guard DebugAutomation.autoMarkWorldOrigin,
              ...
              DebugAutomation.forceCameraWorldOrigin || autoMarkHasUsableCenterHit
        else { return }
    ...
    private func handlePrimaryAction() {
        if appState.canRecord {
            arCapture.requestCoinFTRestart()
        }
        appState.handlePrimaryAction()
    }

    private func shouldEnableRecordingBuffer(for phase: CapturePhase) -> Bool {
        switch phase {
        case .previewUncalibrated, .previewCalibrated, .startCountdown, .recording:
            return true
        case .calibrationGripper, .calibrationWorld:
            return false
        }
    }
}
...
private struct PreviewPane: View {
                ...
                VStack(spacing: 0) {
                    ZStack(alignment: .bottomTrailing) {
                        WorldCalibrationARView(
                            ...
                            renderUltraWidePreview: appState.previewSource == .ultra,
                            videoEncoder: arCapture.h264VideoEncoder,
                            ...
                            onFrameUpdate: { frameState in
                                ...
                                arCapture.applyARWideFrame(frameState, previewSource: appState.previewSource)
                            },
                        ...
                        if appState.previewSource == .depth {
                            ZStack(alignment: .bottomLeading) {
                                Rectangle()
                                    .fill(Color.black)
                                if let image = arCapture.depthPreviewImage {
                                    Image(uiImage: image)
                                        .resizable()
                                        .interpolation(.none)
                                        .scaledToFit()
                                } else {
                                    Text("waiting for arkit depth")
                                        .font(.system(size: 12, weight: .semibold, design: .monospaced))
                                        .foregroundStyle(.white.opacity(0.58))
                                }
                                depthLegend
    ...
    private var previewChrome: some View {
        ZStack {
            VStack {
                HStack {
                    Button {
                        appState.selectPreview(appState.previewSource.next)
                    } label: {
                        ZStack {
                            Circle()
                            ...
                            VStack(spacing: 0) {
                                ...
                                Text(appState.previewSource.shortLabel)
    ...
    private var depthLegend: some View {
        HStack(spacing: 6) {
            ...
            LinearGradient(
                colors: [
                    Color(red: 0, green: 0.22, blue: 1),
                    Color(red: 0, green: 1, blue: 0.78),
                    Color(red: 1, green: 0.92, blue: 0),
                    Color(red: 1, green: 0.05, blue: 0.05),
                ],
                startPoint: .leading,
                endPoint: .trailing
            )
    ...
    private var discardButton: some View {
        Button {
            ...
            Image(systemName: "xmark")
                ...
                .frame(width: 42, height: 34)
                .background(AppPalette.red.opacity(0.12))
                .overlay(
                    RoundedRectangle(cornerRadius: 7)
                        .stroke(AppPalette.red.opacity(0.90), lineWidth: 1)
                )
                .clipShape(RoundedRectangle(cornerRadius: 7))
        }
        .buttonStyle(.plain)
    ...
    private var hasBlockingHardwareError: Bool {
        let phaseStatuses: [String]
        switch appState.phase {
        case .calibrationWorld:
            phaseStatuses = [arCapture.wideStatus, arCapture.capabilityText]
        case .calibrationGripper:
            phaseStatuses = [arCapture.ultraStatus, arCapture.capabilityText]
        default:
            phaseStatuses = [arCapture.wideStatus, arCapture.ultraStatus, arCapture.torchStatus, arCapture.capabilityText]
        }
        let statuses = phaseStatuses
            .joined(separator: " ")
            .lowercased()
    ...
    private var flowStatusText: String {
        ...
        case .calibrationWorld:
            ...
            let arStatus = appState.worldCalibration.arStatusText.lowercased()
            if arStatus.contains("mark pending") || arStatus.contains("mark requested") {
                return appState.worldCalibration.centerDistanceM == nil ? "等待平面" : "标记中"
            }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                ParameterSection(title: "Key Status") {
                    ...
                    StatusRow("record_buf", arCapture.recordingRuntime.bufferText)
                    StatusRow("packet", arCapture.recordingRuntime.latestPacketPreview.summaryText)
                    StatusRow("h264 encoder", arCapture.recordingRuntime.h264EncoderText)
                    StatusRow("h264 spool", arCapture.recordingRuntime.h264SpoolText)
                    StatusRow("capture integrity", arCapture.recordingRuntime.captureIntegrityText)
                }
                ...
                if showAdvanced {
                    ...
                    ParameterSection(title: "Recording V1") {
                        let preview = arCapture.recordingRuntime.latestPacketPreview
                        StatusRow("phase", arCapture.recordingRuntime.phaseText)
                        StatusRow("transport", arCapture.recordingRuntime.transportStatus.displayText)
                        StatusRow("local_file", arCapture.recordingRuntime.localFileText)
                        StatusRow("file_path", arCapture.recordingRuntime.localFileStatus.relativePath)
                        StatusRow("buffer", arCapture.recordingRuntime.bufferText)
                        StatusRow("timebase", preview.timebaseStatus.displayText)
                        StatusRow("aligned_ts", preview.alignedTimestampNS.map(String.init) ?? "--")
                        StatusRow("local_ts", preview.localCaptureTimestampNS.map(String.init) ?? "--")
                        StatusRow("seq", String(preview.sequence))
                        StatusRow("rgb", preview.rgbText)
                        StatusRow("pose", preview.poseText)
                        StatusRow("gripper", preview.gripperText)
                        StatusRow("flags", preview.flagsText)
                    }
