...
def emit_stream_status(
    *,
    ...
    payload = {
        ...
        "colorTimestampMs": round(float(color_frame.get_timestamp()), 3),
        "depthFrameNumber": int(depth_frame.get_frame_number()),
        "colorFrameNumber": int(color_frame.get_frame_number()),
        "rgb": {
            "width": args.rgb_width,
            "height": args.rgb_height,
            "fps": args.rgb_fps,
...
def open_recording_session(
    *,
    ...
    try:
        ...
        depth_file = (output_dir / "depth.raw").open("wb")
...
def run_stream(
    *,
    ...
    def handle_control_request(request: dict[str, Any]) -> None:
        nonlocal recording_session, last_request_marker, latest_command, last_error, last_recorded_stream_frame_id
        ...
        try:
            if command == "start":
                ...
                if recording_session is not None:
                    close_recording_session(recording_session, status="replaced")
                ...
                if recording_session is not None:
                    ...
                    recording_session = None
                    print(
                        json.dumps({"event": "recording_stopped", "requestId": marker, "outputDir": output_text}, ensure_ascii=False),
                        flush=True,
                    )
                else:
    ...
    try:
        ...
        while True:
            ...
            frame_index += 1
            buffered_frame = make_buffered_d435_frame(
                stream_frame_id=frame_index,
...
def main() -> None:
    ...
    config.enable_stream(rs.stream.color, args.rgb_width, args.rgb_height, rs.format.bgr8, args.rgb_fps)
    ...
    if args.stream:
        run_stream(pipeline=pipeline, args=args, device=device, profile=profile, output_dir=output_dir)
        return
